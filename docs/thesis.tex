...
% Colors
% Sourced from GitHub's Primer design system
% See: https://primer.style/primitives/colors
\definecolor{coral}{rgb}{0.737,0.298,0.000} % #bc4c00
\definecolor{blue}{rgb}{0.035,0.412,0.855} % #0969da
\definecolor{purple}{rgb}{0.510,0.314 ,0.914} % #8250df
\definecolor{green}{rgb}{0.102,0.498,0.216} % #1a7f37
\definecolor{lightgreen}{rgb}{0.855,0.984,0.882} % #dafbe1
\definecolor{red}{rgb}{0.812,0.133,0.180} % #cf222e
\definecolor{lightred}{rgb}{1.000,0.922,0.914} % #ffebe9
\definecolor{muted}{rgb}{0.341,0.377,0.416} % #87606a
...
I would like to warmly thank everyone who has given me energy, inspired me, and cheered me on throughout this project, which has been my biggest challenge yet. In particular, I would like to thank George, my supervisor, for his consistent guidance and our inspiring conversations. I would like to thank Michel, my dad, for always being there for me. I would like to thank Libby for helping me find my own way to flourish. Finally, I would like to thank my friends for all their support and kindness.

This thesis concludes my two-year master's degree, which has been a transformative time for me. I am grateful for the opportunities I have been given, and the path they have opened up for me. I am excited to discover the new places it leads to.
...
The rest of this thesis is organized as follows. \autoref{ch:related-work} describes related work on graphs and schemas in academia and industry. In \autoref{ch:data-model}, we formally specify our property graph data model. \autoref{ch:pg-schema} details the design and formalization of our property graph schema model and specifies the rules for schema conformance. \autoref{ch:practice} addresses the translation of our formal rules to graph queries. In \autoref{ch:experiment}, we empirically evaluate our proposed implementation. \autoref{ch:limitations} discusses the limitations of our work and identifies opportunities for future work. \autoref{ch:conclusion} concludes our work with the main takeaways and recommendations.
...
\begin{figure}[t]
  ...
  \caption[Property graphs validated against \autoref{fig:pg-schema}]{Examples of property graphs validated against the schema of \autoref{fig:pg-schema}. Violating nodes and edges are marked and have a red outline.}
...
An example of a schema containing an optional property is shown in \autoref{fig:optional-property}. This example illustrates that optional properties may be omitted, but if they are present, they must have a value of the right type.
...
\begin{figure}[t]
  ...
  \begin{subfigure}[t]{0.8\textwidth}
    ...
    \caption{A property graph consisting of three disconnected nodes. The node with \texttt{id}~:~\texttt{2} violates \autoref{rule:node} because the \texttt{birthday} property value is of the wrong type.}
  \end{subfigure}
...
We focus here on the same three databases we covered in \autoref{ch:related-work}: \emph{Neo4j}, \emph{JanusGraph}, and \emph{TigerGraph}. This choice is motivated by the diversity of schema approaches employed by these systems. Neo4j's schemas are mostly inferred from data, whereas TigerGraph requires an explicit schema, and JanusGraph is schema-optional.
...
\begin{figure}[H]
  \begin{lstlisting}[
    language=Cypher,
    caption={[A set of Cypher statements to create constraints which require the existence of some mandatory properties on nodes]A set of Cypher statements to create constraints which require the existence of some mandatory properties on \texttt{Movie} nodes.},
...
MATCH (n:User)
WHERE NOT all(pKey IN keys(n) WHERE pKey IN keys(propertyTypes) AND apoc.meta.type(n[pKey]) = propertyTypes[pKey])
RETURN n
  \end{lstlisting}
\end{figure}

To check \autoref{rule:simp-edge-endpoints}, we do a \texttt{MATCH} query for every edge label and look at the source and target nodes. For every edge, there must exist a schema edge such that the labels of the source and target in the data match the labels of the source and target in the schema. An example of such a query is shown in \autoref{lst:neo4j-endpoints}.
...
\begin{figure}[H]
  \begin{lstlisting}[
    language=GSQL,
    caption={[A GSQL query to find nodes which are missing an outgoing edge]A GSQL query to find \texttt{Actor} nodes which are missing an outgoing \texttt{ACTED\_IN} edge.},
...
\begin{figure}[H]
  \begin{lstlisting}[
    language=GSQL,
    caption={[A GSQL query to find nodes which are missing an incoming edge]A GSQL query to find \texttt{Post} nodes which are missing an incoming \texttt{CONTAINER\_OF} edge.},
