...
\begin{definition}[Property graph conformance]
  ...
  \begin{enumerate}
    ...
    \item ``Outgoing'' undirected edges are mandatory:\\
    ...
    \item ``Incoming'' undirected edges are mandatory:\\
    $\forall e' \in E_u' \forall n'_1, n'_2 \in N' : \rho'(e') = (n'_1, n'_2) \implies
...
\begin{definition}[Property graph type]
  ...
  \begin{itemize}
    ...
    \item $\rho : (E_d \cup E_u) \to (N \times N)$ is a total function mapping schema edges to pairs of schema nodes;
    \item $\beta : (N \cup E_d \cup E_u) \to \btypes$ is a total function mapping schema elements to element types;
    \item $\gamma_1, \gamma_2 : (E_d \cup E_u) \to (\N \times \N^\infty)$ are total functions mapping schema edges to a cardinality constraint, where $\N$ is defined as $\{0, 1, 2, \ldots\}$ and $\N^\infty$ is defined as $\N \cup \{\infty\}$.
  \end{itemize}
\end{definition}

% TODO: elaborate/example on gamma's
