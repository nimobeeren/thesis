...
\begin{definition}[Basic schema conformance]
  ...
  \label{def:basic-schema-conformance}
  ...
  \begin{enumerate}
    \item\label{rule:basic-conformance-node}
    ...
    \item\label{rule:basic-conformance-edge}
    ...
    \item\label{rule:basic-conformance-out}
    ...
    \item\label{rule:basic-conformance-in}
...
\begin{figure}[t]
  ...
  \begin{subfigure}[t]{0.45\textwidth}
    ...
    \caption{A \texttt{Person} is missing a \texttt{birthday}, which violates rule \ref{rule:basic-conformance-node}.}
  ...
  \begin{subfigure}[t]{0.45\textwidth}
    ...
    \caption{A \texttt{knows} edge is missing a \texttt{since} property, which violates rule \ref{rule:basic-conformance-edge}.}
  ...
  \begin{subfigure}[t]{0.45\textwidth}
    ...
    \caption{The target of a \texttt{knows} edge is not a \texttt{Person}, which violates rule \ref{rule:basic-conformance-edge}.}
  ...
  \begin{subfigure}[t]{0.45\textwidth}
    ...
    \caption{A \texttt{Person} is missing an outgoing \texttt{created} edge, which violates rule \ref{rule:basic-conformance-out}.}
  ...
  \begin{subfigure}[t]{0.45\textwidth}
    ...
    \caption{A \texttt{Person} is missing an outgoing \texttt{knows} edge, which violates rule \ref{rule:basic-conformance-out}.}
  ...
  \begin{subfigure}[t]{0.45\textwidth}
    ...
    \caption{A \texttt{Post} is missing an incoming \texttt{created} edge, which violates rule \ref{rule:basic-conformance-in}.}
...
Next, we revise the definitions of property graph schema and schema conformance, making use of our newly defined cardinality constraints. The following definitions supersede Definition~\ref{def:pg-schema} and~\ref{def:basic-schema-conformance}.
...
An example of a property graph schema with cardinality constraints is given in \autoref{fig:pg-schema}. Note that this is a relaxed variant of \autoref{fig:pg-schema}, which is to say that every graph that conforms to \autoref{fig:pg-schema-basic} also conforms to \autoref{fig:pg-schema}, but not the other way around. Intervals such as $[n, m]$ are written as $n..m$, and $[n, n]$ is written simply as $n$, following notation established in UML~\cite{uml}. Contrary to some data modeling diagram notations, the arrow head is unrelated to cardinality, rather it specifies the direction of an edge.
...
Compared to Definition~\ref{def:basic-schema-conformance}, rules~\ref{rule:conformance-node} and~\ref{rule:conformance-edge} are unchanged, but \ref{rule:conformance-out} and~\ref{rule:conformance-in} now ensure that there are not too few or too many edges of a certain type.
