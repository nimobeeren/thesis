...
\begin{figure}[H]
  \begin{lstlisting}[
    language=Cypher,
    caption={[Creation of constraints which require the existence of some mandatory properties on nodes]Creation of constraints which require the existence of some mandatory properties on \texttt{Movie} nodes.},
...
\begin{figure}[H]
  \begin{lstlisting}[
    language=Cypher,
    caption={[A query to find all nodes that have a label set that is not allowed]A query to find all nodes that have a label set that is not allowed.},
...
\begin{figure}[H]
  \begin{lstlisting}[
    language=Cypher,
    caption={[A query to check if there are any edge labels that are not allowed]A query to check if there are any edge labels that are not allowed. Note that this query returns a Boolean, and does not explain which edges have an illegal label.},
...
\begin{figure}[H]
  \begin{lstlisting}[
    language=Cypher,
    caption={[A query to check if there are any nodes with properties that are not allowed or have the wrong datatype]A query to check if there are any \texttt{User} nodes with properties that are not allowed or have the wrong datatype. If the result is \texttt{true}, there are no violations.},
...
\begin{figure}[H]
  \begin{lstlisting}[
    language=Cypher,
    caption={[A query to find all nodes with properties that are not allowed or have the wrong datatype]A query to find all \texttt{User} nodes with properties that are not allowed or have the wrong datatype.},
...
\begin{figure}[H]
  \begin{lstlisting}[
    language=Cypher,
    caption={[A query that finds all edges where the endpoints have the wrong label]A query that finds all \texttt{ACTED\_IN} edges where the endpoints have the wrong label. Note that the source node \texttt{n} may have additional labels.},
...
\begin{figure}[H]
  \begin{lstlisting}[
    language=Cypher,
    caption={[A query to find nodes that are missing a mandatory edge]A query to find \texttt{ACTOR} nodes that are missing a mandatory \texttt{ACTED\_IN} edge.},
...
\begin{figure}[H]
  \begin{lstlisting}[
    language=Java,
    caption={[A query to find nodes which are missing a mandatory property]A query to find \texttt{User} nodes which are missing a mandatory \texttt{name} or \texttt{userId} property.},
...
\begin{figure}[H]
  \begin{lstlisting}[
    language=Java,
    caption={[A query to find nodes which are missing an outgoing edge]A query to find \texttt{Actor} nodes which are missing an outgoing \texttt{ACTED\_IN} edge. Note that \texttt{ActorDirector} is the concatenation of the \texttt{Actor} and \texttt{Director} labels.},
...
\begin{figure}[H]
  \begin{lstlisting}[
    language=GSQL,
    caption={[A query to find nodes which have a mandatory property with a default value]A query to find \texttt{Movie} and \texttt{User} nodes which have a mandatory property with a default value. All violating nodes are collected in the accumulator variable \texttt{@@violatingNodes}, which is returned at the end.},
...
\begin{figure}[H]
  \begin{lstlisting}[
    language=GSQL,
    caption={[A query to find nodes which are missing a outgoing edge]A query to find \texttt{Actor} nodes which are missing a outgoing \texttt{ACTED\_IN} edge.},
...
\begin{figure}[H]
  \begin{lstlisting}[
    language=GSQL,
    caption={[A query to find nodes which are missing an incoming edge]A query to find \texttt{Post} nodes which are missing a incoming \texttt{CONTAINER\_OF} edge.},
