...
\begin{itemize}
  ...
  \item Schema violations have a moderate impact on validation time for Neo4j, a very significant impact for JanusGraph, and no discernible impact for TigerGraph.
  \item In nearly all scenarios, TigerGraph is much faster than Neo4j, which is much faster than JanusGraph. However, when the data does not conform to the schema, JanusGraph's performance quickly improves, even matching TigerGraph. 
  \item Schema validation can feasibly be performed with Neo4j and TigerGraph on datasets of 20M objects. JanusGraph did not complete on a dataset of 5M objects.
\end{itemize}
