...
Comparing the mean validation time for binary and full variants, we see that they are not significantly different for JanusGraph and TigerGraph, while for Neo4j there is a clear difference. The difference for Neo4j could be explained by the use of Neo4j's schema statistics, which are retrieved in constant time and eliminate the need for some of the queries used in the full variant. However, on the smaller recommendations dataset this advantage is no longer apparent. The lack of difference for JanusGraph and TigerGraph is expected, since they do the same amount of work for both variants.

There is a clear difference between database systems, spanning roughly two orders of magnitude. We are hesitant to draw conclusions from, since indexes and other optimizations can have a large impact on performance. However, these results give an indication of the level of performance that can be expected from an initial implementation.
