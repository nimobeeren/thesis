...
Our schema formalisms come with a number of limitations. First, it is not possible to specify constraints over a graph pattern larger than two neighboring nodes and the edge between them. For example, say we want to allow a person to own a car, but only when they have a driver's license. Such a pattern would consists of three nodes (\texttt{Person}, \texttt{Car}, \texttt{DriversLicense}) and two edges (\texttt{owns}, \texttt{has}), which cannot be expressed.
...
\autoref{fig:subtyping} expresses the constraint of ``every \texttt{Comment} is a reply of exactly one \texttt{Message} (or a subtype of \texttt{Message})''. However, \autoref{fig:subtyping-ours} allows \texttt{Comment}s which are not a reply to anything, or which are a reply to more than one thing. This example shows that our schema formalism cannot be used to model all subtype relations.
...
\begin{figure}[t]
  ...
  \begin{subfigure}[t]{0.45\textwidth}
    ...
    \caption{A schema describing a supertype \texttt{Message} and its two subtypes \texttt{Post} and \texttt{Comment}. The arrow with solid white head indicates a ``subtype of'' relation. This is a fragment of the LDBC SNB schema \cite{angles2020snb}.}
    \label{fig:subtyping}
  \end{subfigure}
  \hfill
  \begin{subfigure}[t]{0.45\textwidth}
    ...
    \caption{An attempt to express the same schema without using subtype relations. Note that the cardinality constraints are more permissive than in \autoref{fig:subtyping}.}
    \label{fig:subtyping-ours}
  \end{subfigure}
  \caption{An example of a subtype relation that cannot be modeled using our schema formalism.}
\end{figure}
