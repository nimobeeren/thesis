...
\paragraph{Validation algorithms.} There are advantages to our approach using graph queries for validation: they are easy to adjust, portable between database versions, and convenient to prototype. However, when performance is the primary goal, a lower-level implementation may be suitable. Specialized validation algorithms and indexes could be developed to achieve significant performance benefits.
% TODO: find areas where algorithms can be beneficial

If a database instance is frequently updated, the cost of scanning the entire graph on every update may be prohibitive. In such a scenario, algorithms for incremental validation would reduce validation time significantly. Since all constraints in our schema formalism have a limited scope, we would only need to scan the fragment of the graph that is affected by the update. Furthermore, the limited scope of our constraints allow validation queries to be easily parallelized and distributed across multiple machines.

\paragraph{Schema-aware query optimization.} \citep{chakravarthy1990logic, buneman1997adding, meier2013semantic}

\paragraph{Query type checking / inference.}

\paragraph{Relation with simulation.}
