...
We also use the functions $\src$ and $\trg$ to refer to the first and second element of a cardinality constraint, i.e. $\src((n_1..m_1, \, n_2..m_2)) = n_1..m_1$ and $\trg((n_1..m_1, \, n_2..m_2)) = n_2..m_2$.
...
\begin{definition}[Property graph conformance]
  ...
  \begin{enumerate}
    ...
    \begin{align*}
      &\forall e' \in E' \forall n_1 \in N \cap \sem{\omega(\src(\rho(e')))}\\
      &\quad\exists^{\src(\gamma(e'))} e \in E \cap \sem{\omega(e')} :\\
      &\quad\quad \src(\rho(e)) = n_1 \wedge \trg(\rho(e)) \in \sem{\omega(\trg(\rho(e')))}
    \end{align*}
    ...
    \begin{align*}
      &\forall e' \in E' \forall n_2 \in N \cap \sem{\omega(\trg(\rho(e')))}\\
      &\quad\exists^{\trg(\gamma(e'))} e \in E \cap \sem{\omega(e')} :\\
      &\quad\quad \src(\rho(e)) \in \sem{\omega(\src(\rho(e')))} \wedge \trg(\rho(e)) = n_2
    \end{align*}
  \end{enumerate}
\end{definition}
