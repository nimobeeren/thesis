...
\begin{definition}[Property graph conformance]
  ...
  \begin{enumerate}
    ...
    \\\forall n_1 \in \sem{\beta(n'_1)} \exists e \in \sem{\beta(e')} \exists n_2 \in \sem{\beta(n'_2)} : \rho(e) = (n_1, n_2)$

    % FIXME: n in [[beta(n')]] doesn't really make sense

    \item To capture that incoming edges are mandatory:\\
    $\forall e' \in E' \forall n'_1, n'_2 \in N' : \rho'(e') = (n'_1, n'_2) \implies
    \\\forall n_2 \in \sem{\beta(n'_2)} \exists e \in \sem{\beta(e')} \exists n_1 \in \sem{\beta(n'_1)} : \rho(e) = (n_1, n_2)$
...
\begin{definition}[Counting existential quantifier]
  The \emph{counting existential quantifier} is defined as follows. For natural numbers $n, m \in \N$ with $n > m$, define
  \begin{itemize}
    ...
    % FIXME: "not exists" might not work on infinite sets
    \item $\exists^{\leq n} x \in X : P(x) \equiv \not \exists^{\geq n+1} x \in X : P(x)$;
...
\begin{definition}[Property graph conformance]
  ...
  \begin{enumerate}
    ...
    $\forall e' \in E' \forall n'_1, n'_2 \in N' : \rho'(e') = (n'_1, n'_2) \implies
    \\\forall n_1 \in \sem{\beta(n'_1)} \exists^{[\ell(e'), u(e')]} e \in \sem{\beta(e')} \exists n_2 \in \sem{\beta(n'_2)} : \rho(e) = (n_1, n_2)$

    \item To capture cardinality constraints on incoming edges::\\
    $\forall e' \in E' \forall n'_1, n'_2 \in N' : \rho'(e') = (n'_1, n'_2) \implies
    \\\forall n_2 \in \sem{\beta(n'_2)} \exists^{[\ell(e'), u(e')]} e \in \sem{\beta(e')} \exists n_1 \in \sem{\beta(n'_1)} : \rho(e) = (n_1, n_2)$
