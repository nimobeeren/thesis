...
\begin{table}[t]
  ...
  \begin{tabular}{ll}
    ...
    \hline
  \end{tabular}
  \caption{Mapping between entity--relationship (ER) and property graph (PG) concepts. Roles and edge direction can be used for similar purposes, but are not equivalent.}
  \label{tab:er-pg}
\end{table}
...
\begin{table}[t]
  ...
  \begin{tabular}{ll}
    ...
    \hline
  \end{tabular}
  \caption{Mapping between entity--relationship (ER) and property graph (PG) schema concepts. *Strictly speaking, a property type $\ptype$ is not a set, but $\sem{\ptype}$ is.}
...
\begin{itemize}
  % \item Effect of schema with fixed scale $\rightarrow$ CovidGraph
  % \item Implementation of exact cardinality constraints $\rightarrow$ CovidGraph
  \item Generalized validation queries / query generation
  \item Relation with simulation
  \item In-engine implementation / efficient validation algorithms / indexes / incremental validation
...
% --> Actually, this seems possible, as long as everything is the same for both Person nodes
...
\paragraph{Mutual exclusivity.} Another type of constraint introduced by \citet{barker1990entity} is mutual exclusivity of edges. For example, a \texttt{Person} may have a \texttt{WORKS\_AT} edge to a \texttt{Bank} or a \texttt{TradingCompany}, but not both.
