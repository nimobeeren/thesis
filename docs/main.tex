...
Next, we revise the definitions of property graph schema and schema conformance, making use of our newly defined cardinality constraints. The following definitions supersede Definition~\ref{def:pg-schema-basic} and~\ref{def:schema-conformance-basic}.
...
Compared to Definition~\ref{def:schema-conformance-basic}, rules~\ref{rule:conformance-node} and~\ref{rule:conformance-edge} are unchanged, but \ref{rule:conformance-out} and~\ref{rule:conformance-in} now ensure that there are not too few or too many edges of a certain type.
