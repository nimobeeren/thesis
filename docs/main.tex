...
It has been proven useful to formally describe the structure of a database [TODO]. Such a description may help to understand what we can do with a database, perform inference, and ensure data integrity. Such a formal description can take the form of a \emph{schema}, which is a collection of rules which a database instance can be validated against.

% Existing schema approaches are local only: no structural constraints

% Bring conceptual and logical model closer together

\section{Property Graphs}

We start by introducing our data model, which is based on the concept of a \emph{property graph} as established by the Working Group for Database Languages~(WG3) as part of ISO/IEC JTC1/SC32~\cite{deutsch2021gpml}. We use this data model in the rest of this document.
