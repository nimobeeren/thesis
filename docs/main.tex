...
% Paragraph seperator
\newcommand{\sep}{\begin{center}\rule{0.33333\textwidth}{0.4pt}\vspace{4pt}\end{center}}
...
The example of \autoref{fig:pg} can be mapped to our formal model as follows:
\begin{align*}
...
An example containing an optional property is shown in \autoref{fig:optional-property}. This examples illustrates that optional properties may be omitted, but if they are present, they must have a value of the right type.
...
\begin{figure}[t]
  ...
  \end{subfigure}
  \caption[A property graph validated against a schema containing an optional property]{A property graph validated against a schema containing an optional property. The node with \texttt{id}~:~\texttt{2} violates \autoref{rule:node} because the \texttt{height} property value has the wrong type.}
