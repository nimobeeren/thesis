...
Define $\exists^{\geq n} x \in X : P(x)$ as $\exists x_1, x_2, \ldots, x_n \in X : P(x_1) \wedge \ldots \wedge P(x_n) \wedge \forall 1 \leq i < j \leq n : x_i \neq x_j$.

% FIXME: "not exists" might not work on infinite sets
Define $\exists^{\leq n} x \in X : P(x)$ as $\not \exists^{\geq n+1} x \in X : P(x)$.

Define $\exists^{[n, m]} x \in X : P(x)$ as $\exists^{\geq n} x \in X : P(x) \wedge \exists^{\leq m} x' \in X : P(x')$,
% FIXME: this is a bit weird
and $\exists^{[n, \infty]} x \in X$ as $\exists^{\geq n} x \in X$.
