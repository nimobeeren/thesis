...
% TODO: gentle introduction, mention motivation of ER diagrams
% TODO: ER doesn't have direction, disambiguate with "arrow" in ER

It has been proven useful to formally describe the structure of a database [TODO]. Such a description may help to understand what we can do with a database, allows us to perform inference and ensure consistency. Such a formal description can take the form of a \emph{schema}, which is a collection of rules which a database instance can be validated against.

A schema should allow modeling of real-world entities and their relationships. A more expressive schema language enables the specification of more complex constraints, although this may come with TODO: greater engineering effort? performance?

% TODO: map ER features to PG concepts

An example of a property graph schema is given in \autoref{fig:pg-schema}. The similarity between property graphs and schemas allows us to visualize and think about them iusing the same mental model. Schema nodes and edges are drawn in the same way as for data graphs, except properties are associated with types rather than concrete values.
...
To formally define property graph schemas and schema conformance, we first assume the existence of a set of property types $\mathcal{T}$. Next, we will introduce a number of supporting concepts.
