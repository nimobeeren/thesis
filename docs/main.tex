...
The concept of property type is similar to a \emph{value set} in the ER model. Value sets and property types are used to specify which values may be associated with an attribute, and which values a property is allowed to have\footnote{Note the subtle difference in terminology: an ER attribute is associated with a value, whereas a property has a name and a value}.
...
We can relate our formalism to the ER model once again. Our definitions of schema nodes and schema edges are analogous to the concepts of \emph{entity set} and \emph{relationship set}, respectively. Entity sets and schema nodes represent classes of entities that have something in common, whereas relationship sets and schema edges represent classes of relationships. A minor difference is that \citet{chen1976entity} postulates that values from different value sets can be equivalent, while we do not have a notion of value equivalence. \autoref{tab:er-pg-schema} summarizes the relationship between the ER model and our schema formalism.
...
\begin{definition}[Basic schema conformance]
  ...
  \begin{enumerate}
    ...
    \begin{align*}
      &\forall n \in N \; \exists n' \in N' : n \in \sem{\omega(n')}
    \end{align*}
    ...
    \begin{align*}
      ...
      \wedge \trg(\rho(e)) \in \sem{\omega(\trg(\rho'(e')))}
    \end{align*}
  \end{enumerate}
\end{definition}
...
\begin{definition}[Schema conformance]
  ...
  \begin{enumerate}
    ...
    \begin{align*}
      &\forall e \in E \; \exists e' \in E' :\\
      &\quad e \in \sem{\omega(e')} \wedge \src(\rho(e)) \in \sem{\omega(\src(\rho'(e')))}\wedge \trg(\rho(e)) \in \sem{\omega(\trg(\rho'(e')))}
    \end{align*}
