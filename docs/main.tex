...
For a given property graph instance, it may be useful to formally describe its structure. Such a description may help to understand what we can do with a database, and allows inferring the type of results. Furthermore, we may want to prescribe some rules that the database should conform to, which ensure consistency and prevent mistakes. TODO: for both these purposes, schema, which is set of rules

We first assume the existence of a set of property types $\mathcal{T}$. We then introduce a number of concepts that lead us to a formal definition of property graph schemas.
...
The similarity between property graphs and schemas is intentional. This allows us to visualize and think about them in a similar ways. In fact, a property graph schema can be simulated by a property graph if we allow properties to take property types as values, i.e. $\ptypes \subseteq \mathcal{V}$. Then we let $\lambda$ and $\pi$ take the role of $\omega$, in the sense that $\lambda(o) = L$ and $\pi(o) = \rtype$ if $\omega(o) = (L, \rtype)$ for all objects $o$ in the property graph.

An example of a property graph schema is given in \autoref{fig:pg-schema}. Schema nodes and edges are drawn in the same way as for data graphs, except properties have types as values.
